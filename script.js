...
function buildChain(words, order) {
  ...
  for (let index = 0; index <= words.length - order; index += 1) {
    const state = words.slice(index, index + order).map((word) => word.id).join("\u0000");
...
function generateMarkov(words, order, targetLength, shouldLoop) {
  ...
  while (output.length < safeLength) {
    const key = output.slice(-safeOrder).map((word) => word.id).join("\u0000");
    const options = (chain.get(key) ?? []).filter((word) => word && !used.has(word.id));
  ...
  return output.slice(0, safeLength).map((word) => word.label).join(" ");
}
...
function generate() {
  ...
  const generated = generateMarkov(words, order, targetLength, allowLoop.checked);
